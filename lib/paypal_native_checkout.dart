...
class PaypalNativeCheckout {
  ...
  // Your access token is optional, if used to capture the payment if the autocapture from client is active
  String accessToken = '';

  // If autocapture from client is active this parameter is required. Your paypalRequestId must be the same sended
  // in the create order api from the server
  ...
  /// This method initialize the native package the android.
  ///
  ...
  /// * [autoCaptureFromClient] This parameter decide if the capture of money after being approved is executed by the client, if is true `accessToken` and `paypalRequestId` must be differente from null or empty.
  ...
  Future<PaypalNativeCheckout> init({
    ...
    assert(autoCaptureFromClient && accessToken != '' && paypalRequestId != '',
        "The accessToken and paypalRequestId can't be empty or null if autoCaptureFromClient is true");
